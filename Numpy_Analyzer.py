...
class DataAnalytics:
    ...
    def __init__(self):
        ...
        DataAnalytics.total_sessions += 1
...
def parse_slice(slice_str, max_len):
    ...
    end = int(parts[1]) if len(parts) > 1 and parts[1].isdigit() else max_len
    return start, end
...
def create_array_menu(analytics):
    ...
    elements = parse_floats(input("Elements: "))
        ...
        while True:
            ...
            if sub_choice == "1":
                try:
                    if arr.ndim == 2:
                        r = int(
                            input(f"Enter Row index (0 to {arr.shape[0]-1}): ")
                        )
                        c = int(
                            input(
                        ...
                        r = int(
                            input(f"Enter Row index (0 to {arr.shape[1]-1}): ")
                        )
                        c = int(
                            input(
                ...
                try:
                    if arr.ndim == 2:
                        ...
                        r_start, r_end = parse_slice(r_slice, arr.shape[0])
                        c_start, c_end = parse_slice(c_slice, arr.shape[1])
                        ...
                        d_start, d_end = parse_slice(d_slice, arr.shape[0])
                        r_start, r_end = parse_slice(r_slice, arr.shape[1])
                        c_start, c_end = parse_slice(c_slice, arr.shape[2])

                        print("\n[Result] 3D Slice:")
                        print(
                            arr[d_start:d_end, r_start:r_end, c_start:c_end]
                        )
                except Exception:
...
def math_operations_menu(analytics):
    ...
    if choice in ["1", "2", "3", "4"]:
        ...
        if len(elements) != arr.size:
            print(f"[-] Size mismatch! Need exactly {arr.size} elements.")
            return
...
def combine_split_menu(analytics):
    arr = analytics.get_array()
...
def stats_menu(analytics):
    ...
    corr_elements = parse_floats(input("Elements: "))
    ...
    while True:
        ...
        if choice == "1":
            ...
            if analytics.get_array() is None:
                ...
                if choice == "2":
                    math_operations_menu(analytics)
                elif choice == "3":
                    combine_split_menu(analytics)
                elif choice == "4":
                    search_sort_filter_menu(analytics)
                elif choice == "5":
                    stats_menu(analytics)
